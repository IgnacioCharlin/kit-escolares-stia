...
<body>
    ...
    <main class="contenedor">
        <h2 id="titulares">Talles Guardapolvo</h2>
        <table class="table table-hover centrado">
		<thead>
    		<tr>
      			<th scope="col" id="titulo-colum">TALLE</th>
      			<th scope="col" id="titulo-colum">CANTIDAD</th>
    		</tr>
  		</thead>
  		<?php
			$sql="SELECT TALLE_KIT, count(*) as CANTIDAD FROM kit_form WHERE ESTADO_KIT ='A'  AND GRADO_KIT  IN ('P1', 'P2','P3', 'P4', 'P5', 'P6', 'P7') GROUP BY TALLE_KIT ORDER BY TALLE_KIT";
			$result=odbc_exec($conectar,$sql)or die(exit("Error en odbc_exec"));
			while ($registro = odbc_fetch_array($result)){
		?>
		<tbody id="datos">
    		<tr>
				<th scope="row"><?php echo $registro["TALLE_KIT"]?></th>
				<td><?php echo $registro["CANTIDAD"]?></td>
			
			<?php
			}
			$sql2="SELECT count(*) as TOTAL FROM kit_form WHERE ESTADO_KIT ='A' AND GRADO_KIT  IN ('P1', 'P2','P3', 'P4', 'P5', 'P6', 'P7')";
	...
	window.onload = function(){
		document.getElementById("talle").style.borderBottom = "solid 1px green";
	}
	</script>
